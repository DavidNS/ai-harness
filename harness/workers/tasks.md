...
## Role
Produce only the bounded tasks phase result.

## Required Inputs
Use only explore_bundle_view, purpose/bundle.json, validated spec.md, design.md, and explorer_scope.
...
- the normal `tasks.json` artifact candidate required by the phase prompt;
  every full-SDD task must include nonempty `source_artifacts` from explorer_scope, and intentionally skipped scope artifacts must be listed in top-level `deferrals` with a reason;
