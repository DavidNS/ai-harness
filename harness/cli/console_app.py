...
from .bootstrap import RUNNER, _default_provider, _parser
...
def _run_and_follow_decisions(namespace: argparse.Namespace, args: list[str], *, request: str | None = None) -> int:
    interactive = bool(getattr(namespace, "_interactive_ui", False)) and sys.stdin.isatty()
    previous_waiting = _waiting_run_ids(namespace.cwd.resolve()) if interactive else set()
    code = _run(args, request=request, verbose=namespace.verbose, dry_run=namespace.dry_run)
    if namespace.dry_run or not interactive:
        return code
...
def _startup_recovery(namespace: argparse.Namespace) -> int | None:
    ...
    while True:
        default_index = next_default_index if next_default_index is not None else (0 if getattr(namespace, "_recovery_blocked", False) else 1)
...
def _startup_ci_preflight(namespace: argparse.Namespace) -> None:
    if getattr(namespace, "skip_warnings", False) or not getattr(namespace, "_interactive_ui", False) or not sys.stdin.isatty():
        return
    repository = namespace.cwd.resolve()
...
def _dispatch_console_action(namespace: argparse.Namespace, command: str, args: list[str], raw_line: str = "") -> int:
    return _controller(namespace).dispatch_console_action(command, args, raw_tail=_legacy_raw_tail(command, args, raw_line))
...
def _branch_args(namespace: argparse.Namespace) -> list[str]:
    ...
    if not getattr(namespace, "_interactive_ui", False) or not sys.stdin.isatty():
...
def _prepare_start_backend(
    ...
    *,
    ...
    if namespace.prompt_file is None and request and interactive:
...
def _resume(
    ...
    *,
    ...
    if answer is None and selected is None and state.get("status") == "waiting_for_user" and getattr(namespace, "_interactive_ui", False) and sys.stdin.isatty():
    ...
    if follow_decisions:
        _refresh_recovery_block(namespace)
    if follow_decisions and not namespace.dry_run and getattr(namespace, "_interactive_ui", False) and sys.stdin.isatty():
