...
class UiStateTests(unittest.TestCase):
    def test_replace_run_list_preserves_existing_selected_run(self) -> None:
        selected = RunView("run-1", "Fix tests", "RUNNING", "SDD", current_phase="EXPLORE_BUNDLE")
        state = select_run(UiState(), selected, ("resume", "cancel"))

        updated = replace_run_list(
            state,
            (RunSummaryView("run-1", "Fix tests", "RUNNING", "EXPLORE_BUNDLE"),),
        )
    ...
    def test_replace_run_list_clears_missing_selected_run(self) -> None:
        selected = RunView("run-1", "Fix tests", "RUNNING", "SDD")
    ...
    def test_append_events_advances_cursor_and_keeps_summary(self) -> None:
        started = event_view(PhaseStarted("run-1", "EXPLORE_BUNDLE"), 1)
        decision = event_view(UserDecisionRequested("run-1", "decision-1", "Choose", ("continue",)), 2)
        ...
        self.assertIn("phase=EXPLORE_BUNDLE", updated.events[0].summary)
