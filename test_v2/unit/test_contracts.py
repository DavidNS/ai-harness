...
class ContractTests(unittest.TestCase):
    def test_commands_queries_events_and_results_use_bundle_phase_terms(self) -> None:
        commands = (
            StartRun("Fix tests"),
            StartRun("Explore", root_bundle="EXPLORE_BUNDLE"),
            ResumeRun("run-1"),
            RetryBundle("run-1", "EXPLORE_BUNDLE"),
            RetryPhase("run-1", "EXPLORE_BUNDLE", "EXPLORE_CONTEXT_PACK"),
            CancelRun("run-1"),
        ...
        events = (
            ...
            PhaseStarted("run-1", "EXPLORE_BUNDLE", "EXPLORE_REQUEST_UNDERSTANDING"),
            PhaseCompleted("run-1", "EXPLORE_BUNDLE", "EXPLORE_REQUEST_UNDERSTANDING"),
            PhaseFailed("run-1", "EXPLORE_BUNDLE", "EXPLORE_CONTEXT_PACK", "failed"),
    ...
    def test_result_dtos_can_be_created_and_serialized(self) -> None:
        ...
        error = ErrorView("E001", "failed", "EXPLORE_BUNDLE", "EXPLORE_CONTEXT_PACK", "2026-07-01T00:00:00+00:00")
        run = RunView(
            run_id="run-1",
            request="Fix tests",
        ...
        results = (
            ...
            ListRunsResult(runs=(RunSummaryView("run-1", "Fix tests", "WAITING_FOR_USER", "EXPLORE_BUNDLE", "EXPLORE_REQUEST_UNDERSTANDING"),)),
            GetRunStateResult("run-1", "WAITING_FOR_USER", "EXPLORE_BUNDLE", "EXPLORE_REQUEST_UNDERSTANDING", decision),
        ...
        self.assertIn('"current_phase": "EXPLORE_REQUEST_UNDERSTANDING"', encoded)
    ...
    def test_result_dtos_reject_raw_domain_objects_and_bad_nested_values(self) -> None:
        ...
        invalid_cases = (
            ...
            lambda: CommandResult(run=RunView("run-1", "Fix tests", "COMPLETED", "EXPLORE_BUNDLE", completed_phases=tuple(p.value for p in _explore_phases())), events=(object(),)),
        )
    ...
    def test_text_and_enum_fields_are_validated(self) -> None:
        ...
        self.assertEqual("EXPLORE_BUNDLE", PhaseStarted("run-1", "  EXPLORE_BUNDLE  ", "EXPLORE_CONTEXT_PACK").bundle)
        self.assertEqual("COMPLETED", RunView("run-1", "Fix", "  COMPLETED  ", "EXPLORE_BUNDLE", completed_phases=tuple(p.value for p in _explore_phases())).status)

        invalid_cases = (
            lambda: StartRun(" "),
            lambda: StartRun("Fix", root_bundle="SDD"),
            lambda: RetryPhase("run-1", "EXPLORE_BUNDLE", "EXPLORE_BUNDLE"),
            lambda: RetryPhase("", "EXPLORE_BUNDLE", "EXPLORE_CONTEXT_PACK"),
            lambda: RetryBundle("run-1", "NOT_A_BUNDLE"),
            lambda: PhaseStarted("run-1", "EXPLORE_BUNDLE", "NOT_A_PHASE"),
