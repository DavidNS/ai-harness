...
class TestPublicationManifest(unittest.TestCase):
    def test_viable_improvement_analysis_is_recorded_and_snapshotted(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("published/explorer-handoff.json", result.artifacts)
            ...
            handoff = json.loads((result.snapshot_path / "published" / "explorer-handoff.json").read_text(encoding="utf-8"))
            self.assertEqual(1, manifest["manifest_version"])
            self.assertEqual("published/explorer-handoff.json", manifest["handoff_artifact"])
            self.assertEqual("explorer_handoff", handoff["kind"])
