...
class StateStoreTests(unittest.TestCase):
    def setUp(self):
        ...
        self.state = RunState("run", "request", "EXPLORE_BUNDLE", Strategy.SDD, Mode.CODE, "modify_code", Complexity.MEDIUM, "local")
        self.store.save(self.state)

    def tearDown(self): self.temp.cleanup()

    def test_mutations_validate_transitions(self):
        self.store.mark_phase_completed("EXPLORE_BUNDLE")
        self.store.mark_phase_started("PROPOSAL_BUNDLE")
        self.assertEqual(self.store.load().current_phase, "PROPOSAL_BUNDLE")
        with self.assertRaises(Exception): self.store.mark_phase_started("DESIGN_BUNDLE")

    def test_resume_rejects_corrupt_artifact(self):
        self.artifacts.write("explore.md", "one")
        self.store.record_artifact("explore.md", "EXPLORE_BUNDLE")
    ...
    def test_waiting_decision_records_request_and_answer(self):
        ...
        prefix = ["EXPLORE_BUNDLE", "PROPOSAL_BUNDLE", "SPEC_BUNDLE"]
        self.store.update(completed_phases=prefix, current_phase="DESIGN_BUNDLE")
        waiting = self.store.record_decision_request(
            DecisionRequest("DESIGN_BUNDLE", "Two designs are viable.", "Should compatibility be preserved?", ("Compatibility is lower risk.",)),
            target_phase="DESIGN_BUNDLE",
        )
    ...
    def test_answer_rejects_selected_option_without_matching_request_option(self):
        ...
        self.store.update(completed_phases=prefix, current_phase="DESIGN_BUNDLE")
        self.store.record_decision_request(
            DecisionRequest("DESIGN_BUNDLE", "A free-form decision is needed.", "Which behavior should be used?", ("No fixed options were supplied.",)),
            target_phase="DESIGN_BUNDLE",
        )
        with self.assertRaises(StateError):
    ...
    def test_phase_escalation_invalidates_downstream_artifacts_and_tasks(self):
        ...
        completed = ["EXPLORE_BUNDLE", "PROPOSAL_BUNDLE", "SPEC_BUNDLE"]
        self.store.update(completed_phases=completed, current_phase="DESIGN_BUNDLE", tasks=[Task("T1", "Task")])
        for name, phase in (("spec.md", "SPEC_BUNDLE"), ("design.md", "DESIGN_BUNDLE"), ("tasks.json", "TASKS_BUNDLE")):
            self.artifacts.write(name, phase)
            self.store.record_artifact(name, phase)
        state = self.store.record_phase_escalation(
            PhaseEscalation("DESIGN_BUNDLE", "SPEC_BUNDLE", "The decision changes requirements."),
            active_graph_phase="DESIGN_BUNDLE",
        )
        self.assertEqual("SPEC_BUNDLE", state.current_phase)
        self.assertEqual(["EXPLORE_BUNDLE", "PROPOSAL_BUNDLE"], state.completed_phases)
        ...
        self.assertEqual("DESIGN_BUNDLE", history[0]["escalation"]["origin_phase"])
        self.assertEqual("SPEC_BUNDLE", history[0]["escalation"]["target_phase"])
        self.assertEqual("DESIGN_BUNDLE", history[0]["escalation"]["active_graph_phase"])
