...
def execute(context: PhaseExecutionContext) -> PhaseResult:
    ...
    context.artifacts.ensure_controller_json(context.run.run_id, explore.CONTEXT_PACK_ARTIFACT, lambda: explore_builders.build_context_pack(context.run, profile, context.artifacts), explore.validate_context_pack)
    return PhaseResult()
