...
def execute(context: PhaseExecutionContext) -> PhaseResult:
    context.artifacts.ensure_controller_json(context.run.run_id, "published/spec-handoff.json", lambda: handoff.build_bundle_handoff("spec", ["spec.json"], "DESIGN_BUNDLE"), handoff.validate_handoff)
    return PhaseResult()
