"""Terminal input and rendering helpers for the v2 UI frontend.

Ported from harness/cli/terminal.py; v1 is scheduled for deletion so the
frontend keeps its own copy. Output targets stdout because the v2 renderer
prints there.
"""
...
class TerminalIO:
    ...
    @property
    def ansi(self) -> bool:
        return self.stdin.isatty() and self.stdout.isatty() and os.environ.get("TERM") != "dumb"
...
class PromptRenderer:
    ...
    def clear_previous(self, rows: int) -> None:
        if rows <= 0 or not self.terminal.ansi:
            return
        print(f"\x1b[{rows}F", end="", file=self.terminal.stdout)

    def clear_line(self, text: str = "", *, end: str = "\n") -> None:
        prefix = "\x1b[2K" if self.terminal.ansi else ""
        print(f"{prefix}{text}", end=end, file=self.terminal.stdout)
